<x-filament-panels::page.simple>
    <div class="flex min-h-screen flex-col items-center justify-center p-6 text-center">
        <canvas id="confetti-canvas" class="pointer-events-none absolute inset-0 z-0 h-full w-full"></canvas>
        ...
        <div class="relative z-10 max-w-md">
            ...
            <p class="mb-8 text-lg text-gray-600 dark:text-gray-400">
                Akaunti yako ya Msimamizi Mkuu kwa **{{ config('app.name', 'WakalaPro') }}** imeundwa kikamilifu!
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const myConfetti = confetti.create(canvas, {
                resize: true,
                useWorker: true // Offload to worker if available
            });

            function celebrate() {
                const duration = 5 * 1000; // Confetti for 5 seconds
                const animationEnd = Date.now() + duration;
                const defaults = { startVelocity: 25, spread: 360, ticks: 50, zIndex: 0, scalar: 0.8 };
